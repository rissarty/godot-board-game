...
@onready var victory_label: Label = $CanvasLayer/Victorypanel/victorylabel
@onready var back_button: Button = $CanvasLayer/Victorypanel/Backtomenubutton
...
@onready var diagsnakelayer: TileMapLayer = $TileMap/Diagsnakelayer
@onready var ladder_tilemaplayer: TileMapLayer = $TileMap/LadderTileLayer
@onready var diag_ladders_layer: TileMapLayer = $TileMap/Diagladdermap
...
var DIRECTIONS: Array[Vector2i] = [
	...
	Vector2i(0, -1), Vector2i(0, 1),  # Straight
...
func is_valid_tile(pos: Vector2i) -> bool:
	return board_tiles.has(pos)

func occupied(pos: Vector2i) -> bool:
	return used_cells.has(pos)
# Centralized occupancy tracker - marks ALL used tiles across ALL spawn types
...
const DIAG_SNAKE_SOURCE_ID: int = 0
...
# long Diag snake
const DIAG_LONG_BODY := Vector2i(2,4)
const DIAG_LONG_HEAD := Vector2i(1,3)
const DIAG_LONG_CONNECTOR :=Vector2i(1,4)
const DIAG_LONG_TAIL :=Vector2i(4,6)
...
# Diagonal SKELETON paths (MAIN diagonal tiles ONLY)
const DIAGONAL_SKELETONS: Array = [
	[99, 83, 77, 65, 55,47,33,29,11],  # Main diagonal spine
	[98,84,76,66,54,48,32,30],  
	[97,85,75,67,53,49,31],
	[96,86,74,68,52,50],
	[95,87,73,69,51],
	[94,88,72,70],
	[93,89,71],
...
const DIAG_LADDER_SOURCE_ID: int =0
const LADDER_HEAD_ATLAS  := Vector2i(1, 7)
const LADDER_BODY_ATLAS  := Vector2i(1, 8) 
const LADDER_TAIL_ATLAS  := Vector2i(1, 9)
const LADDER_DIAG_HEAD := Vector2i(5,6)
const LADDER_DIAG_CONNECTOR := Vector2i(5,7)
const LADDER_DIAG_BODY := Vector2i(6,7)
const LADDER_DIAG_TAIL := Vector2i(8,9)
...
func generate_diagonal_snakes(count: int):
	...
	var diag_patterns = [
		...
		[93,88,89,72,71],
		[82,79,78,63,64,57,56,45,46,35,34,27,28,13,12,9,10],
		[81,80,79,62,63,58,57,44,45,36,35,26,27,14,13,8,9],
		[80,61,62,59,58,43,44,37,36,25,26,15,14,7,8],
		[61,60,59,42,43,38,37,24,25,16,15,6,7],
		[60,41,42,39,38,23,24,17,16,5,6],
		[41,40,39,22,23,18,17,4,5],
		[40,21,22,19,18,3,4],
		[21,20,19,2,3]
	]
...
func add_snake_to_board(path: Array, is_diagonal_snake: bool = false):
	...
	for i in range(path.size()):
		...
		# Skip if straight snake exists (preserve straight)
		if not is_diagonal_snake and board_tiles[pos].get("type") != "empty":
			continue
			
		var info = board_tiles[pos]
		var num = info.num  # MOVED INSIDE LOOP ✓
		...
		info.dest = tail_num
		info.body_index = i
		info.body_index_max = body_count
		info.is_diagonal = is_diagonal_snake
		
		board_tiles[pos] = info
		
		# ONLY collision for straight snakes
		if not is_diagonal_snake:
			used_cells[pos] = true
			
		# FIXED: num now in scope
		if i == 0:
			snake_map[num] = tail_num
			print("Snake: ", num, " → ", tail_num)
...
func generate_snake_path() -> Array:
	...
	return path
# Generate FULL path with connectors from skeleton
func build_diagonal_path(skeleton: Array) -> Array:
	var full_path = []
	
	for i in range(skeleton.size()):
		var main_tile = skeleton[i]
		full_path.append(main_tile)
		
		# Add TWO connectors BEFORE next main tile (except last)
		if i < skeleton.size() - 1:
			var next_main = skeleton[i + 1]
			var connector1 = main_tile - 1      # Left connector  
			var connector2 = next_main - 1      # Right connector (adjusted)
			
			# Validate connectors exist
			if number_to_tile(connector1) in board_tiles and number_to_tile(connector2) in board_tiles:
				full_path.append(connector1)
				full_path.append(connector2)
	
	return full_path.map(number_to_tile)
func random_walk(start: Vector2i, goal: Vector2i, max_steps: int) -> Array:
	...
	path.append(goal)
	return path
func get_valid_neighbors(pos: Vector2i, path_so_far: Array) -> Array:
...
func get_snake_atlas_coords(type: String, pos: Vector2i, path: Array[Vector2i], index: int) -> Vector2i:
	match type:
		"snake_head":
			return HEAD_ATLAS  # STRAIGHT_HEAD_ATLAS (use DIAG_HEAD_ATLAS if diagonal start)
		"snake_tail":
			return TAIL_ATLAS
		"snake_body":
			if index == 1:  # Neck (first body)
				return NECK_ATLAS
			# Detect direction for body
			if index < path.size() - 1:
				var curr_dir = path[index + 1] - pos
				if curr_dir == Vector2i(0, 1):  # Down straight
					return BODY_VERT_ATLAS
				elif abs(curr_dir.x) == 1 and curr_dir.y == 1:  # Diagonal down
					return DIAG_BODY_ATLAS
				elif abs(curr_dir.x) == 1 and curr_dir.y == 0:  # Horizontal connector
					return DIAG_CONNECTOR1_ATLAS
				else:  # Corner/other
					return DIAG_CONNECTOR2_ATLAS
			return STRAIGHT_BODY_ATLAS  # Fallback
	return Vector2i(0, 0)  # Default
...
func draw_snakes_on_layer():
	...
	for pos in board_tiles:
		...
		var tile_type = info.get("type", "")
		
		# FORCE DIAG SPRITES if marked diagonal
		if info.get("is_diagonal", false):
			# Draw ALL diagonal snake parts on DIAG layer
			var atlas = Vector2i(0,0)
			match tile_type:
				"snake_head":
					atlas = DIAG_HEAD_ATLAS        # (1,0)
				"snake_tail":
					atlas = DIAG_TAIL3_ATLAS       # (2,2)
				"snake_body":
					var body_idx = info.get("body_index", 0)
					if body_idx == 1:
						atlas = DIAG_CONNECTOR1_ATLAS  # (2,0) Top connector
					elif body_idx == info.get("body_index_max", 999) - 1:
						atlas = DIAG_CONNECTOR2_ATLAS  # (1,1) Low connector
					else:
						atlas = DIAG_BODY_ATLAS        # (2,1) Main body
				_:
					continue
			
			if diagsnakelayer:
				diagsnakelayer.set_cell(pos, DIAG_SNAKE_SOURCE_ID, atlas)
				print("DIAG ", tile_type, " at ", pos, " → ", atlas)
			continue  # Skip straight layer
		
		# STRAIGHT SNAKES only (original logic)
		elif tile_type in ["snake_head", "snake_tail", "snake_body"]:
			match tile_type:
				"snake_head":
				...
				"snake_body":
					...
					if info.get("body_index") == 1:
						atlas = NECK_ATLAS
					snake_tilemaplayer.set_cell(pos, SNAKE_SOURCE_ID, atlas)
			print("Straight ", tile_type, " at ", pos)

func get_diagonal_atlas(type: String, info: Dictionary) -> Vector2i:
	match type:
		"snake_head":
			return DIAG_HEAD_ATLAS
		"snake_tail":
			return DIAG_TAIL3_ATLAS
		"snake_body":
			var body_idx = info.get("body_index", 0)
			var path_len = info.get("body_index_max", 999)
			
			# Use IF instead of dynamic match
			if body_idx == 1:
				return DIAG_CONNECTOR1_ATLAS  # Top connector
			elif body_idx == path_len - 1:
				return DIAG_CONNECTOR2_ATLAS  # Low connector  
			else:
				return DIAG_BODY_ATLAS         # Main body
	return Vector2i(0,0)


func get_straight_atlas(type: String, info: Dictionary) -> Vector2i:
	match type:
		"snake_head": return HEAD_ATLAS
		"snake_tail": return TAIL_ATLAS
		"snake_body":
			return NECK_ATLAS if info.get("body_index") == 1 else BODY_VERT_ATLAS
	return Vector2i(0,0)

func get_path_for_pos(pos: Vector2i) -> Array[Vector2i]:
	var info = board_tiles.get(pos, {})
	if info.get("type") == "snake_head":
		# Simple: just return single tile for head (fast)
		return [pos]
	elif info.get("type") == "snake_tail":
		# Find head by dest matching
		for p in board_tiles:
			if board_tiles[p].get("dest") == info.num:
				return [p, pos]
		return [pos]
	else:
		# Body: use body_index to guess path length (no loop)
		var path_len = info.get("body_index", 0) + 2
		var path: Array[Vector2i] = []
		for i in path_len:
			var test_pos = pos + Vector2i(0, i)  # Downward guess
			if board_tiles.has(test_pos):
...
func generate_diagonal_ladders(count: int):
	...
	while placed < count:
		...
		if sliced_path.size() >= 2:
			var head = sliced_path[0]  # Visual "head" (top)
			var next_tile = sliced_path[1]
			var allowed_pairs = [[81, 80], [61, 60], [41, 40], [21, 20]]
			...
			if head <= next_tile or (next_tile + 1 == head and not is_allowed_exception):
				print("✗ Skip bad diag ladder: [", head, ",", next_tile, "]")
				continue
...
func ladder_tail_spacing_ok(new_path_pos: Array) -> bool:
	if diag_ladder_paths.is_empty(): return true
	
	# FIX: Use MIN NUMBER position, not path[0]!
	var path_nums = new_path_pos.map(func(p): return board_tiles[p].num)
	var new_tail_num = path_nums.min()  # 68, not 87!
		...
		for new_pos in new_path_pos:
			for old_pos in existing_path:
				...
				if dist < 2.0:
					return false
	return true
func draw_diagonal_ladders():
	if not diag_ladders_layer: return
	...
	for path in diag_ladder_paths:
		...
		for i in range(path_length):
			...
			if i == 0:
				atlas = LADDER_DIAG_HEAD           # Index 0: VISUAL "head" (top)
			elif i == path_length - 1:
				atlas = LADDER_DIAG_TAIL           # Last: VISUAL "tail" (bottom) 
			elif i % 2 == 1:                      
				atlas = LADDER_DIAG_CONNECTOR      # Every 2nd: connectors
			else:
				atlas = LADDER_DIAG_BODY           # Even indices: body
			
			diag_ladders_layer.set_cell(pos, DIAG_LADDER_SOURCE_ID, atlas)
			print("DIAG_LADDER[", i, "] ", board_tiles[pos].num if pos in board_tiles else "???", " → ", atlas)
...
func refresh_hand_for_current_player() -> void:
	...
	# 🔥 GET PARENT WIDTH FIRST! 🔥
	var parent_width = hand_container.get_parent().size.x
	const CARD_WIDTH = 120.0
	const GAP = 50.0  # Increased gap for visibility
	var total_width = hand_limit * (CARD_WIDTH + GAP) - GAP
	var start_x = (parent_width - total_width) * 0.5  # Center offset
...
func _on_swap_target_selected(target_player_color: String) -> void:
	...
	# 🔥 UNSTOPPABLE CHECK - WASTE SWAP IMMEDIATELY 🔥
	if has_permanent(target_player_color, "Unstoppable"):
		rollresult.text = "%s has Unstoppable and can't be swapped!" % target_player_color
		_perm_log("%s tried to Swap %s (Unstoppable) - swap wasted!" % [swap_source_player, target_player_color])
		playerswappanel.visible = false
		swap_source_player = ""
		swap_target_delegate = ""
		end_turn()  # WASTES SWAP + ENDS TURN
		return
...
func start_swap_selection(source_player_color: String) -> void:
	...
	for col in player_nodes.keys():
		if col == source_player_color:
			continue
		var label: String = col
		if has_permanent(col, "Unstoppable"):
			label += " [Unstoppable]"
		var btn := Button.new()
		btn.text = label  # ✅ No more string+bool error!
...
func count_permanent_copies(color: String, name: String) -> int:
	if not player_permanents.has(color):
		return 0
	var count = 0
	for perm in player_permanents[color]:
		if perm.name == name:
			count += 1
	print("DEBUG: %s has %d x %s (array size %d)" % [color, count, name, player_permanents[color].size()])
	return count
...
func _on_permanent_card_bought(card_data: Dictionary, shop_card_node: Node) -> void:
	var current_player := get_current_player_color()
	if current_player == "" or not is_player_turn(current_player):
		return

	var cost := int(card_data.get("shop_cost", 0))
	if not try_spend_money(current_player, cost):  
		print("💸 Insufficient funds for permanent purchase.")
		return
	...
	# Initialize if needed, then add ONCE
	if not player_permanent_cards.has(current_player):
		player_permanent_cards[current_player] = []
	if not player_permanents.has(current_player):
		player_permanents[current_player] = []
...
func try_spend_money(color: String, amount: int) -> bool:
	...
	# ✅ FIX: Total spending power = cash + debt limit
	var total_spending_power: int = current_money + max_debt  # $5 + $10 = $15
...
func _on_dicerollbutton_pressed() -> void:
	...
	if dice_sides <= 0:
		dice_sides = 6
	var dice_roll = randi() % dice_sides + 1
	# DEBUG: Test permanents
	print("G has Bank Card? ", has_permanent("G", "Bank Card"))
	print("G max debt: ", get_max_debt("G"))
	print("G has Odd Space? ", has_permanent("G", "Odd Space Boost"))
	...
	var move_steps = dice_roll
	# Track TOTAL pre-roll bonus (Fibonacci + DoMovei)
	var total_bonus := 0

	# Fibonacci Permanent
	if has_permanent(current_color, "Fibonacci") and dice_roll in _fib_set:
		var fib_bonus = dice_roll
		total_bonus += fib_bonus
		move_steps += fib_bonus
		_perm_log("%s Fibonacci: +%d" % [current_color, fib_bonus])
		#odd space boost
	var odd_space_bonus: int = 0
	if has_permanent(current_color, "Odd Space Boost"):
		var target_pos = min(tile + dice_roll, 100)
		if target_pos % 2 == 1:
			odd_space_bonus = 2
			total_bonus += odd_space_bonus
			move_steps += odd_space_bonus
	# DoMovei Permanent — +2 per card in hand per DoMovei copy
	var extra_from_domovei := 0
	var domovei_copies := count_permanent_copies(current_color, "Domovei")
	if domovei_copies > 0:
		var hand_count: int = player_hands.get(current_color, []).size()
		extra_from_domovei = hand_count * 2 * domovei_copies
		if extra_from_domovei > 0:
			total_bonus += extra_from_domovei
			move_steps += extra_from_domovei
			_perm_log("%s Domovei: +%d" % [current_color, extra_from_domovei])

	# --- Display Result (DoMovei-style!) ---
	if total_bonus > 0:
		rollresult.text = "%s rolled %d (+%d bonus) → %d" % [current_color, dice_roll, total_bonus, move_steps]
	else:
...
# --- Snakes & Ladders (NEW: use generated maps) ---
	...
	# 🔥 ODD SPACE - EXACTLY LIKE SNAKES/LADDERS 🔥
	else:
		odd_space_move(current_color)  # Check if landed on odd tile!
...
func _on_backtomenubutton_pressed():
	...
	# Use change_scene_to_file() - more reliable
	var path = "res://scenes/menuscene.tscn"
	
	# Check if file exists FIRST
	if ResourceLoader.exists(path):
		print("Scene found, changing...")
...
func show_victory_panel(message: String) -> void:
	...
	# Focus button directly (not panel)
	back_button.grab_focus()
	
	# Block ALL input except UI
	set_process_input(false)  # Disable your _input()
#endregion
...
func odd_space_move(color: String) -> void:
	...
	print("🔍 ODD SPACE CHECK:")
	print("  - player_positions['%s'] = %d" % [color, pos])
	print("  - Tile %d odd? %s" % [pos, pos % 2 == 1])
	print("  - Has Odd Space card? %s" % has_permanent(color, "Odd Space Boost"))
	
	if pos % 2 == 1 and has_permanent(color, "Odd Space Boost"):
		var bonus_move = 2
		var final_pos = min(pos + bonus_move, 100)
		var sprite: Node2D = player_nodes.get(color, null)
		
		# Animate bonus
		for i in range(pos + 1, final_pos + 1):
			if tile_map.has(i):
				sprite.position = tile_map[i] + get_offset_if_needed(i, color)
			await get_tree().create_timer(0.2).timeout
		
		player_positions[color] = final_pos
		_perm_log("%s ODD %d → +%d → %d!" % [color, pos, bonus_move, final_pos])
	else:
		print("❌ FAILED: odd=%s OR no card=%s" % [pos % 2 == 1, has_permanent(color, "Odd Space Boost")])
...
func debug_all_positions() -> void:
	print("🎯 POSITION DEBUG:")
	for color in player_positions:
		var pos = player_positions[color]
		print("  %s: tile #%d (odd=%s) VISUAL=???" % [color, pos, pos % 2 == 1])
	print("🎯 END DEBUG")
