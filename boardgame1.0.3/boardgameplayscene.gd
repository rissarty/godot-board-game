...
func _ready() -> void:
	...
	# Setup singleplayer vs pass&play flags & turn_order
	setup_players()
...
func setup_players() -> void:
	# When is_singleplayer is true, we only play R (human) vs B (AI) by default.
	# Change the colours if you want different defaults.
	if is_singleplayer:
		turn_order = ["R", "B"]
		player_is_ai["R"] = false
		player_is_ai["B"] = true
		player_is_ai["G"] = false
		player_is_ai["Y"] = false
	else:
		# Pass & Play: everyone human by default
		turn_order = ["R", "G", "B", "Y"]
		player_is_ai["R"] = false
		player_is_ai["G"] = false
		player_is_ai["B"] = false
		player_is_ai["Y"] = false
# Call at the start of each turn to trigger AI or enable player UI.
func start_turn() -> void:
	var color = get_current_player()
	# reset per-turn flags for UI if needed
	has_rolled_dice = false
	# refresh UI to show the current player's hand/perms
	refresh_hand_for_current_player()
	refresh_permanent_panel_for_current_player()
	populate_shop()
	populate_permanent_shop(color)

	# If this player is AI, run their turn
	if player_is_ai.get(color, false):
		# small deferred call so everything updates visually first
		call_deferred("ai_take_turn", color)
	else:
		# Human player: show UI, wait for player action (no-op here)
		pass

# Simple AI: wait a moment, then roll & move like a human would.
# You can expand this to make decisions (use card, buy, etc).
func ai_take_turn(ai_color: String) -> void:
	# brief delay so player sees "AI's turn"
	await get_tree().create_timer(0.6).timeout

	# attempt to play a card if AI has any (very simple: play first playable)
	# This is optional — comment out if you don't want AI to auto-play cards yet.
	for cd in player_hands[ai_color]:
		# Make sure card has a move_value (consumable card)
		var mv = int(cd.get("move_value", 0))
		if mv != 0:
			# remove from model
			_remove_card_from_player_hand(ai_color, cd)
			# refresh UI if visible (only matters if showing AI's hand)
			if ai_color == get_current_player():
				refresh_hand_for_current_player()
			# apply card effect (consumable)
			apply_card_effect(mv)
			# End card-based action by proceeding to end_turn (if you want card to replace roll, adjust)
			# For now, AI plays card AND still rolls. You can change behavior as desired.
			break

	# small pause then roll
	await get_tree().create_timer(0.4).timeout
	_ai_roll_and_move(ai_color)

# Performs the dice roll + animated move for a specified color (used by AI)
func _ai_roll_and_move(color: String) -> void:
	if has_rolled_dice:
		return
	has_rolled_dice = true

	var sprite = players[color]
	var tile = player_positions[color]
	var dice_sides = int(player_dice_sides.get(color, 6))
	var dice_roll = randi() % dice_sides + 1
	rollresult.text = color + " (AI) rolled a " + str(dice_roll) + " (D" + str(dice_sides) + ")"
...
func end_turn() -> void:
	...
	refresh_permanent_panel_for_current_player()
	refresh_hand_for_current_player()
	# start next player's turn (auto triggers AI if needed)
	start_turn()
...
var ai_player_color := "Blue"
var human_player_color := "Red"

# choose which player colors are human / AI and start the game mode
# default colors: human "R", ai "B"


# Call this from your menu (or before the scene logic runs) to enable singleplayer.
func configure_singleplayer(human_color: String = "R", ai_color: String = "B") -> void:
	is_singleplayer = true

	# set turn order to just the human and AI (adjust order if you want AI first)
	turn_order = [human_color, ai_color]

	# mark AI flags
	for c in player_is_ai.keys():
		player_is_ai[c] = false
	player_is_ai[ai_color] = true
	player_is_ai[human_color] = false

	# make sure model data exists for those players and give starting hands if needed
	create_starting_hand_for_player(human_color)
	create_starting_hand_for_player(ai_color)

	# reset turn index and UI for the start
...
# Call this to switch back to normal pass & play from menu
func configure_pass_and_play() -> void:
